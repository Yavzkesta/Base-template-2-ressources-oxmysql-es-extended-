...
ESX.ShowNotification = function(msg)
 ...
 --ESX.CreateBlipsKingder(100, 0.75, 42, "test", vector3(-819.88, -121.92, 37.45)) ------- à placer entre "CreateThread" & "While true do"
 ---while true do ------------------------------------------------------------------------

ESX.CreateBlipsKingder = function(sprite, taille, colour, textb, coords)

	local kingder = AddBlipForCoord(coords.x, coords.y, coords.z)

	SetBlipSprite(kingder, sprite)
	SetBlipScale(kingder, taille)
	SetBlipColour(kingder, colour)
	SetBlipAsShortRange(kingder, true)

	BeginTextCommandSetBlipName('STRING')
	AddTextComponentSubstringPlayerName(textb)
	EndTextCommandSetBlipName(kingder)
end

-- ESX.CreatePedKingder("PED_TYPE_CIVFEMALE", "a_m_o_acult_02", vector3(-1120.04, 4976.29, 185.51), 355.4)

ESX.CreatePedKingder = function(typeped, blazeduped, coordssped, headingped, invincible, freeze)
	local hash = GetHashKey(blazeduped)
	while not HasModelLoaded(hash) do
	RequestModel(hash)
	Wait(20)
	end
	ped = CreatePed(typeped, blazeduped, coordssped.x, coordssped.y, coordssped.z, headingped, false, true)
	SetBlockingOfNonTemporaryEvents(ped, true)
	if invincible == true or invincible == nil then
		SetEntityInvincible(ped, true)
	end	
	if freeze == true or freeze == nil then
		Wait(200)
...
ESX.ImputKeyboard = function(entryTitle, textEntry, inputText, maxLength)
...
--use 'ESX.RepairVehicleKingder()' dans vos scripts et cela répare le véhicule dans lequel le joueur se trouve.

ESX.RepairVehicleKingder = function()
	if IsPedInAnyVehicle(PlayerPedId()) then 
		local KingderVeh = GetVehiclePedIsIn(PlayerPedId(), false)
		SetVehicleFixed(KingderVeh)
		SetVehicleDirtLevel(KingderVeh, 0.0) 
		ESX.ShowNotification("~g~Votre véhicule à été réparer.")
	else
		ESX.ShowNotification("~r~Vous devez être dans véhicule !")
	end
end
...
RegisterCommand("repair", function()
    ESX.TriggerServerCallback('tesquelgrouppepep', function(group)
        playergroup = group
    end)
    if playergroup == "admin" or playergroup == "helper" then
		ESX.RepairVehicleKingder()
    else 
        ESX.ShowNotification("~r~Non !")
    end
end)
