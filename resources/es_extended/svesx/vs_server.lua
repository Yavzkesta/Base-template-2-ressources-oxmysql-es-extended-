...
RegisterCommand('freezetime', function(source, args)
    ...
    if source ~= 0 then
            if plyGroup ~= nil and (plyGroup == 'admin' or plyGroup == 'helper') then
            ...
            TriggerClientEvent('esx:showNotification', source, '', {255,255,255}, '^8Error: ^1You are not allowed to use this command.')
        end
    else
        freezeTime = not freezeTime
        if freezeTime then
...
RegisterCommand('weather', function(source, args)
    ...
    if source == 0 then
        ...
        if args[1] == nil then
            ...
            local validWeatherType = false
            if args[1] == nil then
                TriggerClientEvent('esx:showNotification', source, '', {255,255,255}, '^8Error: ^1Invalid syntax, use ^0/weather <weatherType> ^1instead!')
            else
                for i,wtype in ipairs(AvailableWeatherTypes) do
                    if wtype == string.upper(args[1]) then
                        validWeatherType = true
                ...
                if validWeatherType then
                    ...
                    TriggerClientEvent('esx:showNotification', source, '', {255,255,255}, '^8Error: ^1Invalid weather type, valid weather types are: ^0\nEXTRASUNNY CLEAR NEUTRAL SMOG FOGGY OVERCAST CLOUDS CLEARING RAIN THUNDER SNOW BLIZZARD SNOWLIGHT XMAS HALLOWEEN ')
                end
            end
        else
            TriggerClientEvent('esx:showNotification', source, '', {255,255,255}, '^8Error: ^1You do not have access to that command.')
...
RegisterCommand('time', function(source, args, rawCommand)
    ...
    elseif source ~= 0 then
            ...
            if tonumber(args[1]) ~= nil and tonumber(args[2]) ~= nil then
                ...
                TriggerClientEvent('esx:showNotification', source, '', {255,255,255}, '^8Error: ^1Invalid syntax. Use ^0/time <hour> <minute> ^1instead!')
            end
        else
            TriggerClientEvent('esx:showNotification', source, '', {255,255,255}, '^8Error: ^1You do not have access to that command.')
